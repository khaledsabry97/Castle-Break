	...
	void Update () {
		if (gameObject.GetComponent <NGUIExample> ().missionPanel.gameObject.activeSelf) {
			foreach (int i in achievementNotification.informedAchievements) {
				if (achievementUIs [i].transform.Find ("Checkmark").GetComponentInChildren <UISprite> ().spriteName != "Check_success") {
					achievementUIs [i].transform.Find ("Checkmark").GetComponentInChildren <UISprite> ().spriteName = "Check_success";
				}
			}
